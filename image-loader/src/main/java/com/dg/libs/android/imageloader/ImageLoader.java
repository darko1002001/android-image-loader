...
import com.dg.libs.android.imageloader.PhotosQueue.PhotoToLoad;
import com.dg.libs.android.imageloader.PhotosQueue.QueueMethod;
...
public class ImageLoader {
    ...
    public ImageLoader(final Context context, final int defaultDrawableId) {
        this(context, defaultDrawableId, true, DEFAULT_HARD_CACHE_SIZE, QueueMethod.STACK);
    }

    public ImageLoader(final Context context, final int defaultDrawableId, final boolean shouldCacheOnSD) {
        this(context, defaultDrawableId, shouldCacheOnSD, DEFAULT_HARD_CACHE_SIZE, QueueMethod.STACK);
    }

    /**
     * @param context application or activity context
     * @param defaultDrawableId the placeholder icon that will be displayed while the photo is loading
     ...
     * @param queueMethod to set if the photos will be loaded FIFO(QUEUE) or LIFO(STACK)
     */
    public ImageLoader(final Context context,
            final int defaultDrawableId,
            final boolean shouldCacheOnSD,
            final int hardCacheCount,
     ...
     */
    public void fetchBitmapAsync(final String url, final ImageLoaderListener listener) {
        fetchBitmapAsync(url, defaultBitmapSize, listener);
    }

    /**
     * This method executes in the background via {@link AsyncTask}. If successful it returns a
     * {@link Bitmap} of the image in the required maximum size specified. It uses the SDcard cache
     * to fetch the image if already downloaded. This method doesn't store the images in the memory
     * cache!
     * 
     * @param url The uri of the photo
     * @param bitmapSizeInPixels the maximum size of the bigger side of the photo in pixels
     * @param listener callback for the result
     */
    public void fetchBitmapAsync(final String url, final int bitmapSizeInPixels, final ImageLoaderListener listener) {
        if (listener == null) {
    ...
    /**
     * Binds a bitmap to the specified imageView. Uses in-memory cache and SDCard cache.
     * 
     * @param url
     * @param imageView
     * @param bitmapSizeInPixels
     */
    public void displayImage(final String url, final ImageView imageView, final int bitmapSizeInPixels) {
        try {
    ...
    private void queuePhoto(final String url, final ImageView imageView, final int bitmapSizeInPixels) {
        // This ImageView may be used for other images before. So there may be
        // some old tasks in the queue. We need to discard them.
        photosQueue.clean(imageView);
        final PhotoToLoad p = new PhotoToLoad(url, imageView, bitmapSizeInPixels);
     ...
     * {@link ImageLoader#fetchBitmapAsync(String, int, ImageLoaderListener)} for an Async Download
     * task or use this one for a different async process.
     * 
     * @param url the uri of the file to load
     * @param bitmapSizeInPixels size the maximum size of the bigger side of the photo in pixels
    ...
    public Bitmap downloadBitmap(final String url, final int bitmapSizeInPixels) {
        ...
        if (shouldCacheOnSD) {
            ...
            try {
                if (f.exists()) {
                    // from SD cache
                    final Options options = Utils.decodeInBounds(f, bitmapSizeInPixels);
                    final Bitmap b = BitmapFactory.decodeFile(f.getPath(), options);
                    if (b != null) {
        ...
        try {
            ...
            try {
                ...
                final BitmapContentHandler handler = new BitmapContentHandler(bitmapSizeInPixels, f);
            ...
            try {
                while (true) {
                    ...
                    if (photosQueue.photosToLoad.size() != 0) {
                        ...
                        final Bitmap bmp = downloadBitmap(photoToLoad.url, photoToLoad.photoSizeInPixels);
                        if (bmp != null) {
                        ...
                        if (tag != null && tag.equals(photoToLoad.url)) {
                            ...
                            try {
                                ...
                                if (loggerEnable) {
                                    Log.w(TAG, "Init the Adapter with an Activity context, NOT application!!!");
                                }
                            }
                        }
                    }
